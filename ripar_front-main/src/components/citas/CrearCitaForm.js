...
function CrearCitaForm({ errores, callback, cselectedEspecialidad = "", cselectedInstitucion = "", cselectedConvenio = "" }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!selectedPaciente || !fechaCita || !horaCita) {
      Swal.fire({
    ...
    <Form onSubmit={handleSubmit} id="cita-form">
      ...
      <Row className="mb-3">
        <Form.Group as={Col} className="mb-3">
          ...
          <Form.Select
            aria-label="Elija una especialidad"
        ...
        <Form.Group as={Col} className="mb-3">
          ...
          <Form.Select
            aria-label="Elija una institución"
      ...
      <Row className="mb-3">
      <Form.Group as={Col} className="mb-3" controlId="ahorro">
          <Form.Label>Ahorro</Form.Label>
          <Form.Control
            type="text"
            value={(parseFloat(detalleConvenio.tarifaParticular || .000) - 
                    parseFloat(detalleConvenio.tarifaMultipreventiva || .000))
                    .toLocaleString('es-ES', { minimumFractionDigits: 3, maximumFractionDigits: 3 })}
            disabled
            readOnly
          />
        </Form.Group>
